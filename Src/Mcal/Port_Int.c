 ...
 *  	LOCAL DATA
 *****************************************************************/
GpioNotification Port_Interrupts[MAX_PORT_NUMBER];
...
void Port_Init(const Port_ConfigType *ConfigPtr)
{
   ...
   for (uint32 index = 0; index < DIO_CONFIG_ARRAY_SIZE; index++)
   {
      ...
      Port_Interrupts[currentPort] = ConfigPtr->pinNotification;
   }
}
...
void GPIOF_Handler(void)
{
   ...
   GpioNotification PortF_CallBack = Port_Interrupts[PortF];

   if (PortF_CallBack != NULL)
   {
      uint32 interruptStatus = GET_REGISTER_POINTER(GPIO_Port_F + GPIORIS);
      for (uint8 index = 0; index < 8; index++)
      {

         if (interruptStatus & 0x1 == HIGH)
         {
            SET_BIT(GET_REGISTER_POINTER(GPIO_Port_F + GPIOICR), index);
            PortF_CallBack(index);
         }
